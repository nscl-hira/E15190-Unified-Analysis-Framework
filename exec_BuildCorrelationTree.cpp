...
int main (int argc, char ** argv)
{
  ...
  for(int cur_run=first_run; cur_run<=last_run; cur_run++)
  {
    ...
    std::string FileOutName(Form("%sCorrelationTree_%04d.root", ExpInfo->GetAnalyzedRootFilePath(), CurrRunInfo->GetRunNumber()));

    //Run the required method(s) /////////
    E15190Analyzer.BuildCorrelationTree(FileOutName.c_str(), evt_amount);
